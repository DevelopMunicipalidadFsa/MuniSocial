...
include_once('Modelos/conexiones.php');
    ...
    $barriosLista=Conexiones::conMunicipalidadDigital()->prepare("SELECT *  FROM Barrios WHERE Activo = 1 ORDER BY Id DESC");

  $barriosLista->execute();

  $Lista =  $barriosLista->fetchAll();
// print_r($Lista);
?>
...
<div class="container">
 <h1 class="text-center mt-3 mb-3 text-uppercase">Gestion de lista de Barrios</h1>
  ...
  <?php if($Lista){ ?>
    ...
    <table class="table table-hover"id="ListaTramites" role="table">
      <thead class="bg-primary text-white" role="rowgroup">
        <tr role="row">
          <td class="text-center" role="columnheader">Nro</td>
          <td class="text-center" role="columnheader">Barrio</td>
          <td class="text-center" role="columnheader">Lote Rural</td>
      ...
      <tbody role="rowgroup">
        ...
        <tr role="row">
          ...
          <td role="cell"><?php if ($value[2] == '' || $value[2] == null) {
            echo "--";
          } else {
            echo $value[2];
          }?></td>
          <td role="cell">
              ...
              <button class="btn btn-warning ml-2" onclick="asignarEdicionTramite('<?php echo $value[0]; ?>','<?php echo $value[1]; ?>','<?php echo $value[2]; ?>')" id="editarTramite" data-bs-toggle="modal" data-bs-target="#modalEditar"><i class="fa fa-edit"></i></button>
          </td>
        </tr>
        <?php endforeach ?>
      </tbody>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5 text-center" id="exampleModalLabel">Agregar un Barrio</h1>
        ...
        <form action="POST" id="altaTramites" onsubmit="altaTramite(); return false">
          <div class="col-md mt-2">
            ...
            <div class="form-floating mt-2">
              ...
              <label for="floatingTextarea">Lote Rural</label>
...
<div class="modal fade" id="modalEditar" tabindex="-1" aria-labelledby="exampleModalLabel"data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5 text-center" id="exampleModalLabel">Editar Barrio</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">

        <form action="POST" id="editarTramites" onsubmit="editarTramite(); return false">
        <input type="" class="form-control" id="editarIdTramite" name="editarIdTramite" autocomplete="off" required>
          ...
          <div class="col-md mt-2">
            ...
            <div class="form-floating mt-2">
            <input type="text" class="form-control" id="editarTramiteDescipcion" name="editarTramiteDescipcion" autocomplete="off" required>
              <label for="editarTramiteDescipcion">Lote Rural</label>
              <small class="text-muted d-flex justify-content-end font-weight-bold">No mas de 250 caracteres</small>
            </div>
          </div>
      <div class="modal-footer">
	...
	*/
    ...
    and (max-device-width: 1024px)  {
      ...
      */
      ...
      td:nth-of-type(2):before { content: "Barrio"; text-transform: uppercase;font-weight: bold;}
      td:nth-of-type(3):before { content: "Lote Rural"; text-transform: uppercase;font-weight: bold;}
